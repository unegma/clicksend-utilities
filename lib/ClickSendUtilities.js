...
const { ClickSendIntegrationError, ClickSendUndefinedCredentialsError } = require('./errors');
...
class ClickSendUtilities {
  ...
  getHeaders = () => {
    let userNameAndPassword = this._clickSendUser + ':' + this._clickSendApiKey;
    if (typeof this._clickSendUser === 'undefined' || typeof this._clickSendUser === 'undefined') {
      throw new ClickSendUndefinedCredentialsError();
    }

    let base64Token = Buffer.from(userNameAndPassword).toString('base64');
    return {
